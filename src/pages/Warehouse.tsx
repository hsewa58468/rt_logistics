...
const getTravelInfo = (fromName: string, toName: string) => {
  ...
  const distance = getHaversineDistance(from.location.lat, from.location.lng, to.location.lat, to.location.lng);
...
const Warehouse: React.FC = () => {
  ...
  const handleAddCargo = (e: React.FormEvent) => {
    ...
    setNewCargo({ type: '', quantity: 0, unit: '板', origin: WAREHOUSES[0].name, destination: WAREHOUSES[1].name });
  };
      ...
      if (existing) {
        ...
        if (newStops.length > 0) {
          ...
          if (lastStop.warehouseName === truck.origin && newStops.length > 1) {
            ...
            // 否則直接加入並補上返回點
            const returnStop: RouteStop = { id: `return-${Date.now()}`, warehouseName: truck.origin, actions: [] };
        ...
        // 全新路徑：出發地 -> 新地點 -> 出發地
        const startStop: RouteStop = { id: `start-${Date.now()}`, warehouseName: truck.origin, actions: [] };
        const returnStop: RouteStop = { id: `return-${Date.now()}`, warehouseName: truck.origin, actions: [] };
        return [...prev, { id: Date.now().toString(), truckId: editingTruckId, truckNumber: truck.number, stops: [startStop, newStop, returnStop] }];
      }
    });
  };
    ...
    setRoutes(prev => prev.map(route => {
      ...
      return {
        ...
        stops: route.stops.map(stop => {
          ...
          if (hasAction) {
            return { ...stop, actions: stop.actions.filter(a => !(a.cargoId === cargoId && a.type === type)) };
          } else {
  ...
  const handleMoveStop = (idx: number, direction: 'up' | 'down') => {
    setRoutes(prev => prev.map(route => {
      ...
      if (targetIdx < 1 || targetIdx >= newStops.length - 1 || idx < 1 || idx >= newStops.length - 1) {
        return route;
      }
  ...
  const handleTruckSelect = (truckId: string) => {
    ...
    setRoutes(prev => {
      ...
      if (!existingRoute) {
        ...
        const initialStop: RouteStop = { id: `start-${Date.now()}`, warehouseName: truck.origin, actions: [] };
        return [...prev, { id: Date.now().toString(), truckId, truckNumber: truck.number, stops: [initialStop] }];
      } else {
        // 2. 已有路徑：檢查起點跟終點
  ...
  return (
    <div className="warehouse-layout">
      ...
      <div className="sidebar-lists">
        ...
        <header className="list-header">
          ...
          <div className="header-actions">
            <Button className="btn-dispatch-main" onClick={() => setIsDispatchModalOpen(true)}>📋 派車單</Button>
            <Button className="btn-save-main" onClick={() => setIsSaveModalOpen(true)}>儲存檔案</Button>
            <Button className="btn-history-main" onClick={() => setIsHistoryModalOpen(true)}>歷史紀錄</Button>
          </div>
        </header>
            ...
            />
             ...
             {editingTruckId && (
              <div className="route-builder">
                <div className="route-builder-header" onClick={() => setIsRouteBuilderExpanded(!isRouteBuilderExpanded)} style={{ cursor: 'pointer' }}>
                  ...
                  <div className="add-stop-controls" onClick={(e) => e.stopPropagation()}>
                    <select 
                      ...
                      value=""
                    >
                      <option value="" disabled>新增停靠點...</option>
                      {WAREHOUSES.map(w => (
                ...
                {isRouteBuilderExpanded && (
                  ...
                  {currentRoute?.stops.map((stop, sIdx) => {
                    ...
                    const travel = prevStop ? getTravelInfo(prevStop.warehouseName, stop.warehouseName) : null;
                      ...
                      <React.Fragment key={stop.id}>
                        {travel && travel.timeMin > 0 && (
                          <div className="travel-time-step">
                            ...
                            <span className="time-text">預估車程：{formatTime(travel.timeMin)} ({travel.distance.toFixed(1)} km)</span>
                          </div>
                        )}
                        ...
                        <div className="stop-card-header">
                          ...
                          <div className="stop-title-info">
                            <span className="stop-location">{stop.warehouseName}</span>
                             <div className="stop-load-badge">
                               ...
                               {opTime > 0 && <span className="op-time-badge"> | ⏳ 作業: {opTime} 分鐘</span>}
                             </div>
                          </div>
                          <div className="stop-action-controls">
                            ...
                            <button className="btn-remove-stop" onClick={() => handleRemoveStop(stop.id)}>×</button>
                          </div>
                        </div>
                            ...
                            <div className="action-items">
                              {cargoList
                                .filter(c => {
                                  ...
                                  const isPickedInCurrentStop = stop.actions.some(a => a.cargoId === c.id && a.type === 'pickup');
                                  return c.origin === stop.warehouseName && (status === 'pending' || isPickedInCurrentStop);
                                })
                                .map(c => (
                                  <button 
                                    key={c.id} 
                            ...
                            <div className="action-items">
                              {cargoList
                                .filter(c => {
                                  ...
                                  const isPickedUp = allStops.slice(0, sIdx).some(ps => 
                                    ps.actions.some(pa => pa.cargoId === c.id && pa.type === 'pickup')
                                  );
                                  const isDroppedOffBefore = allStops.slice(0, sIdx).some(ps => 
                                    ps.actions.some(pa => pa.cargoId === c.id && pa.type === 'dropoff')
                                  );
        ...
        <section className="list-section">
          ...
          <ul className="data-list-refined">
            {cargoList
              .filter(item => locationFilter.includes(item.origin) || locationFilter.includes(item.destination))
              .map((item) => {
              const status = checkCargoStatus(item.id);
              const statusText = status === 'assigned' ? '已送達' : status === 'transit' ? '配送中' : '待處理';
              const statusClass = status === 'assigned' ? 'assigned' : status === 'transit' ? 'transit' : 'pending';
              ...
              return (
                <li key={item.id} className={`cargo-item-complex ${status === 'assigned' ? 'assigned-opacity' : ''}`}>
                  <div className="cargo-main-info">
                    ...
                    {assignedTruck && (
                      <span className="assigned-truck-badge">
                              ...
                              {stop.actions.map((action, aIdx) => {
                                ...
                                return (
                                  <div key={aIdx} className={`action-tag ${action.type}`}>
                                    {action.type === 'pickup' ? '⬆️ 載貨' : '⬇️ 卸貨'}: {cargo?.type}
                                  </div>
                                );
                              })}
                            </div>
      ...
      {/* 儲存檔案光箱 */}
      <Modal isOpen={isSaveModalOpen} onClose={() => setIsSaveModalOpen(false)} title="儲存當前設定">
      ...
      {/* 歷史紀錄光箱 */}
      <Modal isOpen={isHistoryModalOpen} onClose={() => setIsHistoryModalOpen(false)} title="歷史紀錄讀取">
              ...
              {historyRecords.map(record => (
                <li key={record.id} className="history-item" onClick={() => handleLoad(record)}>
                  ...
                  <div className="history-actions">
                    ...
                    <button className="btn-delete-record" onClick={(e) => handleDeleteRecord(record.id, e)}>刪除</button>
                  </div>
                </li>
              ))}
            </ul>
                            ...
                            <React.Fragment key={stop.id}>
                              ...
                              <div className="dispatch-stop-node">
                                <div className="node-marker">
                                  ...
                                  {sIdx < route.stops.length - 1 && <span className="node-line"></span>}
                                </div>
                                <div className="node-content">
                                  <div className="node-location">
                                    {stop.warehouseName}
                                    {getStopOperationTime(stop, cargoList) > 0 && (
                                      <span className="dispatch-op-tag"> (⏳ {getStopOperationTime(stop, cargoList)}m)</span>
                                    )}
                                  </div>
                                  <div className="node-actions">
                                    {stop.actions.map((action, aIdx) => {
                                      const cargo = cargoList.find(c => c.id === action.cargoId);
                                      return (
                                        <div key={aIdx} className={`dispatch-mini-tag ${action.type}`}>
                                          {action.type === 'pickup' ? '載' : '卸'} {cargo?.type}
                                        </div>
                                      );
                                    })}
                                    {stop.actions.length === 0 && <span className="no-actions-text">無作業</span>}
                                  </div>
                                </div>
                              </div>
                            </React.Fragment>
                        ...
                        <div className="dispatch-route-footer">
                          🏁 總時數：{(() => {
                            ...
                            for (let i = 1; i < route.stops.length; i++) {
                              totalMin += getTravelInfo(route.stops[i-1].warehouseName, route.stops[i].warehouseName).timeMin;
                            }
