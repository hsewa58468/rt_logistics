...
const InventoryMng: React.FC = () => {
  ...
  const handleAddItem = async (e: React.FormEvent) => {
    ...
    setNewItem({ warehouse_label: '', part_number: '', item_name: '', original_quantity: 0 })
  ...
  const handleShip = async (e: React.FormEvent) => {
    ...
    if (histErr) { console.error(histErr); return }
  ...
  const handleDeleteWarehouse = async (w: Warehouse) => {
    if (!window.confirm(`確定刪除倉庫「${w.warehouse_name}」？該倉庫的庫存資料也將一併刪除。`)) return
  ...
  return (
    <div className="inventory-page">
      ...
      <div className="warehouse-tabs">
        {warehouses.map(w => (
          <div key={w.warehouse_id} className={`tab-item ${activeWarehouseId === w.warehouse_id ? 'active' : ''}`}>
            <button
      ...
      <div className="inventory-content">
        ...
        <table className="inventory-table">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={7} className="loading-cell"><span className="loading-spinner" /></td></tr>
            ) : items.length === 0 ? (
              <tr><td colSpan={7} className="table-empty">此倉庫尚無資料</td></tr>
            ) : (
              items.map(item => (
                <tr key={item.item_id}>
      ...
      {/* 新增倉庫 Modal */}
      <Modal isOpen={isAddWarehouseOpen} onClose={() => setIsAddWarehouseOpen(false)} title="新增倉庫">
